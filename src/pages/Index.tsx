...
import { Dna, User, LogOut, Crown, Settings } from 'lucide-react';
...
export default function Index() {
  ...
  const { latestAnalysis, refresh: refreshLatestAnalysis } = useLatestAnalysis();
  const [activeTab, setActiveTab] = useState<'scan' | 'history' | 'progress' | 'scanner' | 'coach' | 'routines' | 'streaks' | 'settings'>('scan');
  ...
  const [premiumTrigger, setPremiumTrigger] = useState<'scan_limit' | 'routine' | 'progress' | 'scanner' | 'coach' | 'analytics' | 'history'>('scan_limit');
...
Generated by Dermatological Intelligence
  ...
  const handleUpgrade = (trigger: 'scan_limit' | 'routine' | 'progress' | 'scanner' | 'coach' | 'analytics' | 'history' = 'scan_limit') => {
    ...
    <div className="min-h-screen pb-24">
      ...
      <main className="px-4 space-y-6">
        ...
        {activeTab === 'coach' && (
          !isPremium ? (
            <FirstScanRequired 
              onGoToScan={() => handleUpgrade('coach')} 
              featureName="AI Skin Coach"
              description="Access neural-powered personalized guidance that adapts in real-time to your skin's current state, environmental conditions, and progress goals—like having a dermatologist in your pocket."
              isPremiumFeature={true}
            />
          ) : hasCompletedFirstScan ? (
            <AISkinCoach 
